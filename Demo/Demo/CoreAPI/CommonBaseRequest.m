...
@implementation CommonBaseRequest
...
+ (instancetype)requestWithSuccessBlock:(CommonRequestSuccessBlock)successBlock failureBlock:(CommonRequestFailureBlock)failureBlock {
    return [[[self class] alloc] initWithSuccessBlock:successBlock failureBlock:failureBlock];
}

- (void)startCompletionBlockWithSuccess:(CommonRequestSuccessBlock)success failure:(CommonRequestFailureBlock)failure {
...
- (void)constructURL {
    _url = [NSString stringWithFormat:@"%@%@", [self commonNetworkDomain], [self requestURLPath]];
}
...
- (NSURLSessionTask *)sessionTaskForError:(NSError *__autoreleasing *)error {
    MTRequestMethod method = [self requestMethod];
...
- (AFHTTPRequestSerializer *)requestSerializer {
    ...
    switch ([self requestSerializerType]) {
        case MTRequestSerializerTypeHTTP: {
            requestSerializer = [AFHTTPRequestSerializer serializer];
        } break;
        case MTRequestSerializerTypeJSON: {
    ...
    if (error) {
        ...
        if (json) {
            if ([json isKindOfClass:[NSString class]]) {
                ...
                if (resultData) {
                    [self toolHandleData:resultData errCode:errorCode];
                } else {
                    DLog(@"服务器返回数据为null");
...
- (MTRequestMethod)requestMethod {
    return MTRequestMethodGET;
}

- (MTRequestSerializerType)requestSerializerType {
    return MTRequestSerializerTypeJSON;
}
...
- (NSString *)requestURLPath {
    NSAssert([self isMemberOfClass:[CommonBaseRequest class]], @"子类必须实现requestURLPath");
    return nil;
}
...
- (void)toolHandleData:(id)data errCode:(NSInteger)errCode {
    
    ///< block 回调
    id handleResult = [self handleData:data errCode:errCode];
    if (self.successBlock) {
        self.successBlock(errCode, data, handleResult);
    }
    
    ///< 代理回调
    if (self.delegate && [self.delegate respondsToSelector:@selector(requestDidFinishLoadingWithData:errCode:)]) {
        [self.delegate requestDidFinishLoadingWithData:handleResult ? handleResult : data  errCode:errCode];
    }
}
